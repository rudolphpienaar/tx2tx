...
- On GNOME, a Shell extension can expose global pointer position over DBus; set `TX2TX_GNOME_POINTER=1` to enable it.
- When running with `sudo`, preserve the session bus environment so the helper can access DBus.
