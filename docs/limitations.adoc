...
Different pointer-coordinate providers (`helper`, `gnome`, `gnome_bridge`) can behave differently under load or specific session topologies.

Impact:

- Boundary detection stability can differ across setups.

=== GNOME truth-bridge is GNOME-specific

The `gnome_bridge` provider depends on a GNOME Shell extension exposing a local Unix socket stream.

Impact:

- It improves coordinate authority on GNOME Wayland but does not generalize to other compositors.
- Extension lifecycle (enabled/disabled/restarted) directly affects provider availability.
...
- For GNOME Wayland, prefer `gnome_bridge` over helper-integrated pointer coordinates when edge drift appears.
