...
def main():
    ...
    server_thread = threading.Thread(target=stream_output, args=(server, "[SERVER]"), daemon=True)
    server_thread.start()
    time.sleep(2)
    ...
    client_thread = threading.Thread(target=stream_output, args=(client, "[CLIENT]"), daemon=True)
    client_thread.start()
    time.sleep(2)
